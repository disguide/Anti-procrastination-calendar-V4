...
const Calendar: React.FC<CalendarProps> = ({ selectedDate, onSelectDate }) => {
    ...
    // Generate localized weekday headers starting from Monday
    const weekDayHeaders = useMemo(() => {
        const days = [];
        const d = new Date(2024, 0, 1); // Jan 1 2024 is a Monday
        for (let i = 0; i < 7; i++) {
            // 'narrow' gives single letter usually, but some locales might give more. 
            // We ensure it's short.
            ...
            return (
                <div className="grid grid-cols-7 gap-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
                    {weekDayHeaders.map((d, i) => (
                        <div key={`${d}-${i}`} className="text-center text-[10px] font-bold text-zinc-500 dark:text-zinc-600 py-1">{d}</div>
                    ))}
                    {days.map((d, i) => {
                        ...
                        return (
                            ...
                            >
                                <span className="text-[8px] font-bold uppercase opacity-60 leading-none mb-0.5">
                                    {d.toLocaleDateString(language, { weekday: 'short' }).charAt(0)}
                                </span>
    ...
    const selectedDateString = selectedDateObj.toLocaleDateString(language, { weekday: 'short', month: 'short', day: 'numeric' });
    const currentMonthName = currentDate.toLocaleString(language, { month: 'short' });
            ...
            <div className="max-w-2xl mx-auto">
                ...
                <div className="flex justify-between items-center px-4 py-2">
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <span className="text-sm font-bold text-primary-600 dark:text-primary-400 truncate capitalize">
                            {selectedDateString}
                        </span>
                    </div>

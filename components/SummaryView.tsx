...
const SummaryView: React.FC<SummaryViewProps> = ({ tasks, onBack, onRollOver, onBatchUpdate, theme, darkMode }) => {
  ...
  const handleSliderChange = (id: string, newProgress: number) => {
    ...
    updateTaskState(id, {
      ...
      isRolledOver: !isNowCompleted // Auto-toggle rollover based on completion
    });
  };
  ...
  const toggleRollover = (id: string) => {
    const current = getTaskState(id);
    updateTaskState(id, {
      isRolledOver: !current.isRolledOver
    });
  };
        ...
        <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
           ...
           {tasks.map(task => {
             ...
             return (
               <div 
                  key={task.id} 
                  className={`relative p-4 flex items-center gap-4 transition-all overflow-hidden group ${
                     state.isCompleted
